...
ICM42688::ICM42688(TwoWire *bus, uint8_t address)
{
  if (i2c_dev)
    delete i2c_dev;
  i2c_dev = new Adafruit_I2CDevice(address, bus); // I2C bus
...
int ICM42688::enableDataReadyInterrupt()
{
  ...
  if (writeRegister(INT_CONFIG, INT_PULSE_100us) < 0)
  { // setup interrupt, pulse
    // if (writeRegister(INT_CONFIG,INT_HOLD_ANY) < 0) { // setup interrupt, hold, any read operation
    return -1;
  }
  if (writeRegister(INT_SOURCE0, UI_DRDY_INT1_EN) < 0)
  { // set to data ready
    return -2;
  }
  return 1;
...
int ICM42688::disableDataReadyInterrupt()
{
  // use low speed SPI for register setting
  _useSPIHS = false;
  if (writeRegister(INT_SOURCE0, RESET_DONE_INT1_EN) < 0)
  { // set to reset done (disable interrupt)
    return -1;
  }
  return 1;
}
...
int ICM42688::writeRegister(uint8_t subAddress, uint8_t data)
{
  ...
  uint8_t buffer_w[2] = {subAddress, data};
  i2c_dev->write(buffer_w, 2);
  delay(10);
...
{
  uint8_t buffer_w[1] = {subAddress};
  if (i2c_dev->write_then_read(buffer_w, 1, dest, count))
  {
    return 1;
  }
  else
